function [data] = ppc_raw_b2(studyDir)
% PPC_RAW_b3.M %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
% set up paths/filenames
rawDataDir = fullfile(studyDir,'raw','b2');
cleanDataDir = fullfile(studyDir,'clean');
stdColsPy = ['python /Users/wem3/Developer/zach/csv_ops/standardize_columns.py'];
baseFile = fullfile(studyDir,'munge','b2_trial_headers.csv');
tmpFile = fullfile(cleanDataDir,'tmp.csv');
b2_trialFile = fullfile(cleanDataDir,'b2_trial_data.csv');
...
system(['grep "main_expt" ',tmpFile,' | cut -d'','' -f 1,3,5,6,7,8,17,18,19,20,21,22,23,24 > ',b2_trialFile]);
...
for s = 1:height(b2_demo)
    ...
    subject.subID = b2_demo.subID{s};
    subject.drift = b2_demo.drift(s);
    subject.age = b2_demo.age(s);
    subject.sex = b2_demo.gender(s);
    subject.race = b2_demo.race(s);
